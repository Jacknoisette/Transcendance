...
function modifySprite(sprite : PIXI.Sprite, posx : number, posy : number, width : number, height : number){
	let cpy_sprite : PIXI.Sprite = new PIXI.Sprite(sprite.texture);
	cpy_sprite.x = posx;
	cpy_sprite.y = posy;
...
async function draw_image(sprite : PIXI.Sprite, obj_ball : any, bsize : number) {
...
async function write_score(bsize : number, nbr : number, posx : number, posy = (app.screen.height / 7)){
...
function draw_background(){
    ...
    const bgContainer = app.stage.getChildByLabel("background") as PIXI.Container;
	...
	function createCornerGradient(x: number, y: number, radius: number) {
...
async function draw_game(rungame : any){
	...
	if (rungame.vision == true){
		for (let obj of rungame.ball_real_array_futur){
			if (obj.touch == true) drawFutur(obj, rungame.ball_size * 7, "#0033ff", "#0033ff", !rungame.negative); //draw_image(obj, rungame.ball_size * 2, imgsrc.bounce_ballImg);
			else if ((obj.x <= rungame.ball_size + 8 || obj.x >= WIDTH - (rungame.ball_size + 8))) drawFutur(obj, rungame.ball_size * 7, "#ff0000", "#f71d1dcd", !rungame.negative); //draw_image(obj, rungame.ball_size * 2, imgsrc.kill_ballImg);
			else  drawFutur(obj, rungame.ball_size * 7, "#e5ff00", "#ecef8f", !rungame.negative); //draw_image(obj, rungame.ball_size * 2, imgsrc.futur_ballImg);
		}
	}
